...
class Arena: # The Fighting Arena
    ...
    def __init__(self, root : tk.Tk):
        ...
        self.root.bind('<Up>', self.scroll_vertical)
        self.root.bind('<Down>', self.scroll_vertical)
        self.root.bind('<Right>', self.scroll_horizontal)
        self.root.bind('<Left>', self.scroll_horizontal)
    ...
    def mouse_click(self, event):
        ...
        # Move a unit here
        if selected[0] and self.backtracking_result != [0] * 100 and selected[0] == self.backtracking_result[-1].id:
    ...
    def mouse_move(self, event):
        ...
        if selected:
            ...
            if selected[0] in self.canvas.find_withtag('field') and self.mouse_hold and self.combat_phase == True:
                ...
                print(isinstance(field.occupied, self.Unit))
                ...
                if not field.occupied in your_units:
            ...
            #             self.can_attack_unit(my_unit, unit)
    ...
    def can_attack_unit(self, attacker , attacked):
        if self.backtracking_result == [0] * 100:
            self.pathfinding(attacker.pos, attacked.pos, attacker.current_actions)
            print(attacker.pos.id, attacked.pos.id, attacker.current_actions)
            print(self.backtracking_result)
    ...
    def refresh_unit(self, unit):
        ...
        self.pathfinding(unit.pos, random.choice(unit.pos.neighbor_fields), unit.current_actions)
        for field in self.unit_walked_fields:
    ...
    def pathfinding(self, start, end, actions):
